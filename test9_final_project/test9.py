...
def apply_map_config(args, config, config_path):
    ...
    print(f"ジャイロ単位: {GYRO_UNIT}/s")
    print(f"PF収縮半径: {PF_EROSION_RADIUS_PX}px")
    print(f"経路優先: {len(ROUTE_POINTS)}点, 幅={ROUTE_WIDTH_PX:.1f}px, 経路外重み={OFF_ROUTE_WEIGHT:.2f}")
    print("総距離校正: " + ("無効" if args.target_distance_px is None else f"{args.target_distance_px:.1f}px"))
...
def load_preprocessed_map(img_path):
    ...
    if radius > 0:
        ...
        passage_pf = passage
    binary_for_pf = np.where(passage_pf, 255, 0).astype(np.uint8)
    dist_map = ndimage.distance_transform_edt(passage_pf)
    maximum = float(dist_map.max())
    if maximum > 0:
        dist_map /= maximum
    print(f"前処理済み地図: passage_ratio={passage.mean():.3f}, PF用={passage_pf.mean():.3f}")
    return binary, binary_for_pf, dist_map


def build_route_mask(shape):
    """JSONのroute_pointsから、優先するL字経路のマスクを作る。"""
...
def redraw_all_paths():
    ...
    for file_name in file_list:
        ...
        for i in range(1, len(df)):
            ...
            # 今回の実験経路（L字廊下）を優先するが、部屋を完全な壁にはしない。
        ...
        if estimated_positions:
            ...
            ax.scatter(START_X, START_Y, s=50, zorder=5,
...
# ============================================================
    ...
    title = "test9: 管理棟L字経路優先 + SmartPDR + パーティクルフィルタ"
    if japanize_matplotlib is None:
        title = "test9: management route-aware SmartPDR + PF"
    ax.set_title(title)
