...
for file_name in file_list:
    ...
    for i in range(1, len(df)):
        ...
        # 簡易的な中間地点チェック
        mid_x = (particles[:, 0] + new_particles[:, 0]) / 2
        mid_y = (particles[:, 1] + new_particles[:, 1]) / 2

        # 移動先、または中間地点が壁なら衝突とみなす
        hit_wall = is_in_wall(new_particles[:, 0], new_particles[:, 1]) | is_in_wall(mid_x, mid_y)
