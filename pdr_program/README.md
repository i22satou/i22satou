...
- `run_evaluation.py` — **卒論第7章の比較実験を一括実行する評価ハーネス**。計測一覧表の`eval`の行
  ...
  地図ごとに1回の前処理(経路帯の自動抽出など)の時間は、歩ごとの処理時間と分けて別の表にする。
...
- `../削除候補/`は不要と判断した過去のプロトタイプ・実験結果の移動先(削除はしていない)。
  旧版の`pdr_pf_clickstart.py`、初期のL字検証の`test7.py`・`L.png`もここにある(詳細は`../memo/file_cleanup.md`)。
- `../memo/`・`../CLAUDE_MEMO.md`は調査記録で、複数トピックにまたがるためリポジトリ直下に残している。
- 卒論の原稿は`../卒論/卒論原稿.txt`。
